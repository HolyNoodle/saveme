...
const ActorPicker: React.FunctionComponent<ActorPickerProps> = ({value, onChange}) => {
  ...
  return (
    <Picker
      ...
      style={{width: '70%'}}>
      {Object.keys(actorComponents).map(
        (key) =>
          key && (
            <Picker.Item
              key={key}
...
interface EditableActorConfigProps {
  edit: boolean;
  value: TimelineItem;
  onChange: (item: TimelineItem) => void;
}
const EditableActorConfig: React.FunctionComponent<EditableActorConfigProps> = ({edit = false, value, onChange}) => {
  const {t} = useTranslation();
